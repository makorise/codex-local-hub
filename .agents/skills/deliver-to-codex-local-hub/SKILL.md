...
description: Deliver an existing screenshot or raster image to the Codex Lookout delivery inbox for phone review. Use when the user asks to send, publish, or show an image, screenshot, mockup, or visual result in Codex Lookout or its mobile dashboard.
---

# Deliver to Codex Lookout
...
The helper uses Codex Lookout's bundled architecture-specific Node.js runtime when the app is installed, falling back to an existing `node` command only for source-development environments. It never installs or changes Node.js. It accepts PNG, JPEG, WebP, and GIF files up to 20 MB, copies the image into Codex Lookout's sandbox-safe staging outbox, and prints a JSON result; it never removes or changes the source image. `CODEX_TASK_DESK_OUTBOX` can override the staging directory when the host uses a custom location.

After staging:

1. Verify that the JSON result reports `staged: true` and that its `path` exists.
2. Tell the user the image is ready in Codex Lookout's **Delivery inbox**. The running host imports it during the next refresh.
