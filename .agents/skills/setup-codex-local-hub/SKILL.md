...
When this stale-cache recovery is needed and the native `CFBundleShortVersionString` is older than `0.2.15`, a core-only update is temporary and does not repair the updater itself. Complete one data-preserving full host upgrade during the same run, even if the newest core starts successfully. Verify the native bundle version separately from `--effective-version`; both must be current before declaring the update path repaired.
