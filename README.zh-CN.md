...
```text
使用 $skill-installer 安装 https://github.com/makorise/codex-local-hub/tree/main/.agents/skills/setup-codex-local-hub，然后在当前任务中使用 $setup-codex-local-hub：安装或更新 Codex 瞭望台、安装图片交付 Skill、启动程序并验证手机访问。优先使用最新正式版，否则从源码构建；保护已有文件，不要修改我的全局 Node.js 环境。
```
...
<p align="center">
  <img src="docs/assets/social-preview/codex-local-hub-social-preview.png" width="100%" alt="Codex 瞭望台：用手机查看和控制 Mac 上的 ChatGPT Codex 任务">
</p>
...
Codex 瞭望台是一个开源的 **Codex 手机控制台、任务监控器和 macOS 远程助手**。打开 Mac 客户端，扫描二维码，即可在同一局域网内查看 ChatGPT 桌面版里的 Codex 任务进度、最近对话、长程目标和图片交付结果，并从手机继续发送提示词。
...
<p align="center">
  <img src="docs/assets/screenshots/mac-host.zh-CN.png" width="720" alt="Codex 瞭望台 Mac 宿主程序，展示服务状态、示例二维码、局域网地址和启动控制">
</p>
...
<p align="center">
  <img src="docs/assets/screenshots/desktop-home.zh-CN.png" width="920" alt="Codex 瞭望台主页，展示用量、任务数量、筛选项、任务状态，以及尚未选择任务时的对话区域">
</p>
...
<p align="center">
  <img src="docs/assets/screenshots/desktop-dashboard.zh-CN.png" width="920" alt="Codex 瞭望台任务页面，展示选中的任务、最近对话、目标状态、队列数量、最后问题和消息输入框">
</p>
...
<table>
  <tr>
    <td width="50%"><img src="docs/assets/screenshots/mobile-dashboard.zh-CN.png" width="100%" alt="Codex 瞭望台手机主页，展示当前用量和任务状态"></td>
    <td width="50%"><img src="docs/assets/screenshots/mobile-conversation.zh-CN.png" width="100%" alt="Codex 瞭望台手机任务页面，展示最近消息、目标状态、队列数量、最后问题和消息输入框"></td>
  </tr>
  <tr>
...
```text
使用 $setup-codex-local-hub 更新我现有的 Codex 瞭望台源码仓库和已安装程序。只有仓库完全干净时才允许快进；保留全部本地修改，运行完整测试，构建内置双架构运行时的通用程序，备份旧版本后完成替换、重新启动并验证手机访问。不要让我手动下载安装包。
```
...
![Codex 瞭望台工作原理图：安装到 Mac、读取本地 Codex 状态、通过局域网同步，并从手机双向控制](docs/assets/how-it-works.zh-CN.svg)
...
4. **从手机反向控制。** 新提示词、队列操作、停止或归档任务、项目管理操作都会通过 Codex 原生控制协议回传到 Mac。系统只保留用于监控的少量最近内容，不会建立第二份完整聊天档案。

当前版本只在局域网内工作，数据只在 Mac 与同一可信 Wi-Fi 下的设备之间传输，不经过 Codex 瞭望台的公网中继。
...
- 一键恢复被中断或暂停的任务
- 在手机上停止或归档任务、从 Codex 移除项目；不会删除电脑上的工程文件
- 查看长程 goal、执行时长和当前状态
- 查看 Codex 用量、重置时间，以及由这台 Mac 本地采集的最近 7 天趋势
- 显示当前 Codex 账号的脱敏名称，不向工作台传输完整邮箱、账号 ID 或凭据
...
**交付信箱**本身已经内置在 Codex 瞭望台中。使用上面的一句话安装方式时，系统会自动安装配套的 [`deliver-to-codex-local-hub`](.agents/skills/deliver-to-codex-local-hub) Skill。它会告诉 Codex 如何检查现有截图或图片，并把它安全地放入交付通道。Skill 使用 App 内置的 Node.js，不会增加全局运行时依赖。原图不会被移动或修改，信箱只保留最近 20 张受支持的图片；看完后可用紧凑的“清空”按钮删除信箱副本，不影响原图。
...
使用时请保持 Codex 瞭望台运行，图片会在下一次刷新时出现在**交付信箱**。支持 PNG、JPEG、WebP 和 GIF，单张不超过 20 MB。Codex 如何发现和调用 Skill，可参考 [OpenAI 官方 Skill 文档](https://learn.chatgpt.com/docs/build-skills)。
...
Mac 程序每天最多检查一次最新的 GitHub 正式版，不需要 Codex 瞭望台自建更新服务器。只有远端语义版本严格高于当前版本时才会提示。普通更新只下载经过 SHA-256 校验的核心包（本地服务与网页），原子切换后重启服务；新核心无法启动时会自动回退。只有 Swift 宿主必须变化时才使用完整通用 DMG。草稿、预发布版、相同版本和降级版本都会被忽略，也可以随时点击“检查更新”手动检查。
...
可以。锁屏不会停止同步；Mac 休眠、关机或退出 Codex 瞭望台后会暂时无法访问。

> 旧版本曾使用 Codex Local Hub 名称；内部标识保持不变，以兼容现有升级。
