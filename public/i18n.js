const messages = {
  'zh-CN': {
    ...
    'composer.startedHint': '消息已送达，任务正在启动', 'composer.queuedHint': '消息已排队；桌面通道可用后开始', 'composer.delayed': '启动稍慢，消息仍安全保留在队列中', 'composer.waitingDesktop': '已排队，等待 Codex 桌面端接管；不会另开执行器', 'composer.failure': '发送失败，消息已放回输入框',
  ...
  en: {
    ...
    'composer.startedHint': 'Message delivered. The task is starting.', 'composer.queuedHint': 'Queued; it will start when Codex Desktop is available.', 'composer.delayed': 'Startup is taking longer; your message is still safe in the queue.', 'composer.waitingDesktop': 'Queued for Codex Desktop; no second runner will be opened.', 'composer.failure': 'Could not send. Your message was restored to the input.',
